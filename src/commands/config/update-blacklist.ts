import { updateBlacklistMessage } from "@/data/handler";
...
/**
 * Set blaclist command content from message id.
 * @version 1.0.0
 */
export class SetBlacklistCommand extends NauticalCommand {
    public constructor(context: NauticalCommand.Context, options: NauticalCommand.Options) {
        super(context, {
            name: "setblacklist",
            description: "Set Blacklist command content from message id.",
    ...
    public override registerApplicationCommands(registry: NauticalCommand.Registry) {
        const command = new SlashCommandBuilder()
            ...
            .addStringOption((option) =>
                option
                    .setName("messageid")
                    .setDescription("The message id to set the Blacklist command content from.")
                    .setRequired(true),
            );
    ...
    public async chatInputRun(interaction: NauticalCommand.ChatInputCommandInteraction) {
        ...
        try {
            ...
            updateBlacklistMessage(messageId, content);

            return await interaction.reply("The blacklist message has been updated.");
        } catch {
